...
//  static DialogManager get instance => _getInstance();
  ...
  // 静态私有成员，没有初始化
  static late DialogManager _instance;

  ///弹窗数据（待显示）
  late List<DialogBean> _dialogList;
...
//  DialogBean showDialogBean;
  ...
  late List<DialogBean> _hasShowDialogList;
  ...
  DialogManager._internal() {
    // 初始化
    _dialogList = <DialogBean>[];
    _hasShowDialogList = <DialogBean>[];
  ...
  void add(DialogBean bean) {
    ...
    switch (bean.dialogPriority) {
      ...
      case DialogPriority.highClearAll:
        ...
        _dialogList.removeWhere(
          (item) => (item.priority ?? 0) < (bean.priority ?? 0),
        );
        break;
      default:
        break;
    ...
    if (!hasDialog) {
      ...
      if (clearBean == null ||
          (bean.priority ?? 0) >= (clearBean.priority ?? 0)) {
        _dialogList.add(bean);
      }
    }
    //根据优先级进行排序:降序
    _dialogList.sort((a, b) => (b.priority ?? 0).compareTo((a.priority ?? 0)));
  }
...
//    }
    ...
    if (NavigatorManager().isTopDialog() && _hasShowDialogList.isNotEmpty) {
      ...
      if ((bean = _getHighClearDialogBean(beans: _dialogList)) != null &&
          _canShow(bean) &&
          ((hasShowDialogBean!.priority ?? 0) < (bean!.priority ?? 0))) {
    ...
    //查找当前页面可显示的弹窗数据
    DialogBean? showDialogBean = _dialogList.firstWhere(
      (bean) => _canShow(bean),
      orElse: () => DialogBean(createDialogWidget: () => Container()),
    );
  ...
  List<DialogBean> _getAllDialogBean() {
    List<DialogBean> all = <DialogBean>[];
